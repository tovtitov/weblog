...
func RemoveBase64Content(str string) string {
	...
	pos_long := 0
	pos_curr := strings.Index(str[pos_long:], ";base64,")
	for pos_curr > -1 {
		pos_curr += 8
		pos2 := strings.Index(str[pos_curr:], `"`)
		if pos2 > -1 {
			pos := pos_curr + pos2
			img := str[pos_curr:pos]
			str = strings.Replace(str, img, "...", 1)
			pos_long = pos_curr + 4
			pos_curr = strings.Index(str[pos_long:], ";base64,/")
			if pos_curr > -1 {
				pos_curr += pos_long
